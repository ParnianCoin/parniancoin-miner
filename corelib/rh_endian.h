...
#if defined(_WIN32) && !defined(PARNIANMINER_PLATFORM_GPU)
#include <stdlib.h>
...
#define PARNIANMINER_TO_LITTLE_ENDIAN32(val) val
#define PARNIANMINER_TO_BIG_ENDIAN32(val) RH_swap_u32(val)
#define PARNIANMINER_TO_LITTLE_ENDIAN64(val) val
#define PARNIANMINER_TO_BIG_ENDIAN64(val) RH_swap_u64(val)

#elif BIG_ENDIAN == BYTE_ORDER

#define PARNIANMINER_TO_LITTLE_ENDIAN32(val) RH_swap_u32(val)
#define PARNIANMINER_TO_BIG_ENDIAN32(val) val
#define PARNIANMINER_TO_LITTLE_ENDIAN64(val) RH_swap_u64(val)
#define PARNIANMINER_TO_BIG_ENDIAN64(val) val
...
#define PARNIANMINER_SWAP_VAL(A,B, T) {T t = A; A = B; B = t;}
...
static inline void be32enc_vect(uint32_t *dst, const uint32_t *src, uint32_t len)
{
    ...
    for (i = 0; i < len; i++)
        dst[i] = PARNIANMINER_TO_BIG_ENDIAN32(src[i]);
}
