...
bool IsValidPassword(const string& inPW)
{
    ...
    // constant-time compare (does not leak the password length/prefix by timing)
    size_t n = inPW.length() > g_apiPW.length() ? inPW.length() : g_apiPW.length();
    unsigned char diff = (unsigned char)(inPW.length() != g_apiPW.length());
    for (size_t i = 0; i < n; i++)
    {
        unsigned char a = i < inPW.length() ? (unsigned char)inPW[i] : 0;
        unsigned char b = i < g_apiPW.length() ? (unsigned char)g_apiPW[i] : 0;
...
void ProcessMinerFile(Json::Value responseObject, socket_ptr sock)
{
    try
    {
        ...
        else
        {
                ...
                __stricmp(fn.c_str(), "config.xml") == 0)
            {
                ...
                if (data.empty())
                {
                    PrintOut("Remote API Error. Empty file content\n");
                    return;
                }
    ...
    {
        PrintOut("Error parsing api.miner_file\n");
    }
}
...
void MiniWeb_Connection(socket_ptr sock)
{
    try
    {
        ...
        *data = 0;
        ...
        // keep one byte for the terminating zero (the old code could write past the buffer)
        size_t length = sock->read_some(boost::asio::buffer(data, MaxBuffer - 1), error);
        if (length >= MaxBuffer)
            length = MaxBuffer - 1;
        data[length] = 0;

        // do not write api requests (they may contain the api password) to the log
...
{
    // By default the api is only reachable from this computer
    tcp::endpoint ep = g_apiRemote ? tcp::endpoint(tcp::v4(), port)
                                   : tcp::endpoint(boost::asio::ip::address_v4::loopback(), port);
    acceptor = new tcp::acceptor(io_service, ep);

    PrintOut("Remote API started on %s:%d%s\n", g_apiRemote ? "0.0.0.0" : "127.0.0.1", port,
             g_apiPW.length() ? "" : " (read-only)");
    for (;;)
    {
