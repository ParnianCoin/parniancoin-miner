...
ParnianWorkPackage::~ParnianWorkPackage()
{
...
ParnianWorkPackage::ParnianWorkPackage(const ParnianWorkPackage& c)
{
    m_jobID = c.m_jobID;
    m_ntime = c.m_ntime;
...
U64 ParnianSolution::GetCurrentEvaluatingNonce()
{ 
    PARNIANMINER_ASSERT(_eval_current_result_index < m_results.size());
    return m_results[_eval_current_result_index];
}

void ParnianSolution::SetCurrentEvaluatingNonceIndex(U32 i)
{ 
    PARNIANMINER_ASSERT(i < m_results.size());
    _eval_current_result_index = i; 
}


void ParnianWorkPackage::Init(const string& job, const h256& prevHash, const string& coinbase1, const string& coinbase2, const string& nTime, bool cleanWork, const string& nonce1, U32 nonce2Size, U64 extranonce, const string& server)
{
...
void ParnianWorkPackage::ComputeWorkDiff(double& diff)
{
    if (diff == 0)
    {
        double d64, dcut64;
...
void ParnianWorkPackage::ComputeTargetBoundary(h256& boundary, double& diff, double diffMultiplyer)
{
...
bool ParnianWorkPackage::Eval(ParnianSolution* solPtr)
{
...
extern void ParnianHashV3(void *state, const void *input);
bool ParnianWorkPackage::IsSame(ParnianWorkPackage* work)
{
    PARNIANMINER_ASSERT(m_fullHeader.size())
    return (m_jobID == work->m_jobID && 
            m_ntime == work->m_ntime && 
            m_fullHeader == work->m_fullHeader);
}

U32  ParnianWorkPackage::GetDeviceTargetUpperBits()
{
    return *(U32*)(m_deviceBoundary.data() + 28);
}

U64  ParnianWorkPackage::GetDeviceTargetUpperBits64()
{
    return *(U64*)(m_deviceBoundary.data() + 24);
}

bool ParnianWorkPackage::IsEmpty()
{
...
h256 ParnianWorkPackage::RebuildNonce(U64 nonce)
{
    h256 solution;
    h256 solutionTmp;
...
void ParnianWorkPackage::ComputeTargetBoundary()
{
    m_deviceDiff = m_workDiff;
...
U64 ParnianWorkPackage::ComputeNonce2(U32 nonce2)
{
...
void ParnianWorkPackage::UpdateHeader()
{
    ...
    {
        PARNIANMINER_EXIT_APP("Private key length is too long. Please chose a mining key with encryption type secp256k1.\n");
    }
    ...
    if (m_fullHeader.size() != ParnianHeaderSizeV5)
    {
    ...
    h32 ntime(m_ntime);
    U32 ntimeIdx = 48;
    ntimeIdx = (ParnianHeaderSizeV5 / 4) - 2;
...
ParnianWorkPackage* ParnianWorkPackage::Clone() 
{ 
    return new ParnianWorkPackage(*this); 
}