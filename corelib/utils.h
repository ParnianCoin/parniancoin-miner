...
#define PARNIANMINER_ONLY_ONCE_CODE_BEGIN() \
{  \
...
#define PARNIANMINER_ONLY_ONCE_CODE_END() \
    } \
}
...
//
//  Debug/Assert
#define PARNIANMINER_PRINT_EXCEPTION_EX(MSG_TEXT, EXTRA_TEXT) PrintOut("%s %s\n", MSG_TEXT, EXTRA_TEXT);

#define PARNIANMINER_EXIT_APP(MSG) \
    { \
        if (*MSG) \
...
#define PARNIANMINER_CL_EXEC(code) \
{  \
    cl_int res = 0; \
    try \
    { \
        res = code;  \
        PARNIANMINER_ASSERT(res == 0);  \
    } \
