...
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_INT(g_cputhrottling, 0)
...
void RandomHashCPUMiner::UpdateWorkSize(U32 absoluteVal)
{
    ...
    if (m_globalWorkSize && m_globalWorkSize != absoluteVal)
        PARNIANMINER_EXIT_APP("Cpu miner cannot set an arbitrary worksize\n");    
...
bool RandomHashCPUMiner::init(const ParnianWorkSptr& work)
{
...
PrepareWorkStatus RandomHashCPUMiner::PrepareWork(const ParnianWorkSptr& workTempl, bool reuseCurrentWP)
{
...
void RandomHashCPUMiner::SendWorkPackageToKernels(ParnianWorkPackage* wp, bool requestPause)
{
    ...
    for (auto& data : m_cpuKernels)
    {
        ...
        PARNIANMINER_ASSERT(wp->m_fullHeader.size() <= sizeof(kernelData->m_header.asU8));

#ifdef RH_RANDOMIZE_NONCE2
        if (!wp->m_isSolo)
        {
            kernelData->m_nonce2 = merssen_twister_rand_fast(&m_rnd32);
            U64 n264 = ParnianWorkPackage::ComputeNonce2((U32)kernelData->m_nonce2);
        ...
        PARNIANMINER_ASSERT(wp->m_jobID.length() < sizeof(kernelData->m_workID)-1);
...
void RandomHashCPUMiner::SetWork(ParnianWorkSptr _work)
{ 
