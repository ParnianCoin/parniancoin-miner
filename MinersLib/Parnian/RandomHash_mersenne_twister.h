...
inline uint32_t merssen_twister_rand_fast_partial_slow(mersenne_twister_state* state, const int MaxPrecalc)
{
    if (state->index == MERSENNE_TWISTER_SIZE)
    {
        const U32 A = PARNIANMINER_CEIL(MaxPrecalc, 4);

        {
            const U32 Pe = PARNIANMINER_CEIL(MaxPrecalc + 1 + MERSENNE_TWISTER_PERIOD, 4);
    ...
    {
        const U32 A = PARNIANMINER_CEIL(MaxPrecalc, 4) * 2;
        const size_t C = state->index;
        PARNIANMINER_ASSERT(C < A);
    }
#endif
...
#else //#if defined(PARNIANMINER_ENABLE_SSE4)
...
#endif //#if defined(PARNIANMINER_ENABLE_SSE4)
