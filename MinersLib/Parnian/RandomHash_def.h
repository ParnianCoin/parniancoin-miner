...
#ifdef PARNIANMINER_DEBUG
    #undef RH_DISABLE_RH_ASSERTS
#endif

#if defined(PARNIANMINER_ENABLE_SSE4)
    #if defined(PARNIANMINER_NO_SSE4)
...
#if !defined(_WIN32_WINNT) && !defined(PARNIANMINER_ENABLE_SSE4)
...
#ifdef PARNIANMINER_PLATFORM_CPU
...
#ifndef PARNIANMINER_PLATFORM_GPU // CPU
    #define RH_DECL_HOST
...
#elif defined(PARNIANMINER_PLATFORM_CUDA)
...
#elif defined(PARNIANMINER_PLATFORM_OPENCL)
    #define KERNEL_LOG(...) printf(__VA_ARGS__);
    #define KERNEL0_LOG(...) {if (KERNEL_GET_GID() == 0) {printf(__VA_ARGS__);}}
...
#define PARNIANMINER_T64(x)    ((x) & uint64_t(0xFFFFFFFFFFFFFFFF))
...
#define RH_ASSERT PARNIANMINER_ASSERT
#else
...
#ifdef PARNIANMINER_DEBUG_DEV
  #ifdef PARNIANMINER_DEBUG
    #define RH_DISABLE_RH_ASSERTS
  #endif
  #include "MinersLib/Parnian/RandomHash_DEV_TESTING.h"
#endif
