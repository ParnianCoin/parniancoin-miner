...
public:
    ...
    void SetWork(ParnianWorkSptr _wp);
    ...
    using RequestNewWorkFunc = std::function<void(ParnianWorkSptr, GenericCLMiner*)>;    
    ...
    ParnianWorkSptr GetWork() const { Guard l(m_farmData.m_workMutex); return m_farmData.m_work; }
    
    void ReconnectToServer(uint32_t minerRelIndex) override;
    void RequestNewWork(ParnianWorkSptr wp, GenericCLMiner* miner) override;
