...
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_STRING("logfilename", g_logFileNameDummy, "General", "Set the name of the log's filename.\nNote: the log file will be overwritten every time you start parnianminer");
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_STRING("configfile", g_configFileNameDummy, "General", "Xml config file containing all config options.\nAll other command line options are ignored if config file given.");
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_BOOL("cpu", g_useCPU, "Gpu", "Enable the use of CPU to mine.\nex '-cpu -cputhreads 4' will enable mining on cpu while gpu mining.");
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT("testperformance", g_testPerformance, "Debug", "Run performance test for an amount of seconds", 0, 300)
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT("testperformancethreads", g_testPerformanceThreads, "Debug", "Amount of threads to use for performance test", 0, 256)
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT("processpriority", g_setProcessPrio, "General", "On windows only. Set miner's process priority.\n0=Background Process, 1=Low Priority, 2=Normal Priority, 3=High Priority.\nDefault is 3.\nWARNING: Changing this value will affect GPU mining.", 0, 10);
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT("memoryboost", g_memoryBoostLevel, "Optimizations", "This option will enable some memory optimizations that could make the miner slower on some cpu.\nTest it with -testperformance before using it.\n1 to enable boost. 0 to disable boost.\nEnabled, by default, on cpu with hyperthreading.", 0, RH_OPT_UNSET+1);
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT("sseboost", g_sseOptimization, "Optimizations", "This option will enable some sse4 optimizations.\nIt could make the miner slower on some cpu.\nTest it with -testperformance before using it.\n1 to enable SSe4.1 optimizations. 0 to disable.\nDisabled by default. ", 0, 2);
PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_BOOL("restarted", g_restared, "*", "");
...
class GlobalMiningPreset
{
    public:
        ...
        bool        IsInDevFeeMode() { return false; } // dev fee removed in parnianminer
        bool        DetectDevfeeOvertime();
        bool        UpdateToDevModeState(string& connectionParams);
        float       m_devfeePercent = 0.0f; // dev fee removed in parnianminer
