...
void CLMinerBase::UpdateWorkSize(U32 absoluteVal)
{
        ...
        if (m_gpuInfoCache->setGpuThreadCount != U32_Max)
        {
            ...
            else
            {
                ...
                if (m_gpuInfoCache->isOpenCL)
                    ...
                    m_globalWorkSize = PARNIANMINER_FLOOR(m_globalWorkSize, 8); ////round GWS to 8
            }
    ...
    {
        PARNIANMINER_ASSERT(m_gpuInfoCache->maxWorkSize != 0);
...
bool CLMinerBase::init(const ParnianWorkSptr& work)
{
...
bool CLMinerBase::ShouldInitialize(const ParnianWorkSptr& work)
{
...
bool CLMinerBase::BuildKernels(const ParnianWorkSptr& work)
{
#ifndef RH_COMPILE_CPU_ONLY
    //build programs
    ...
    for (int i = 0; i < codes.size(); ++i)
    {
        ...
        if (codeCrc != m_lastCodeCRC[i])
        {
            ...
            catch (cl::Error const& _e)
            {
                ...
                {
                    PARNIANMINER_PRINT_EXCEPTION_EX("OpenCL Error.", _e.what());
                }
...
cl::Kernel  CLMinerBase::GetKernel(U32 codeID, const char* name)
{
    for (auto& k : m_kernels[codeID])
        if (k.first == name)
            return k.second;
    PARNIANMINER_EXIT_APP("Kernel does not exists");
    return cl::Kernel();
}
