...
#define PARNIANMINER_MAKE_ARCH_NAME(devname, gpuToken, archname, var) if (stristr(devname.c_str(), gpuToken)) {var = string(gpuToken) + "_" + archname;}
...
#ifdef PARNIANMINER_ENABLE_SSE4
    if (g_sseOptimization > 2)
        g_sseOptimization = 2;

#if defined(PARNIANMINER_COND_SSE4)
    g_sseOptimization = 0;
#endif
...
#else //#ifdef PARNIANMINER_ENABLE_SSE4
...
#endif //#ifdef PARNIANMINER_ENABLE_SSE4
}
	...
	for (unsigned j = 0; j < platforms.size(); ++j)
	{
		...
		for (U32 di=0; di < devices.size(); di++)
		{
            ...
            if (device.getInfo<CL_DEVICE_AVAILABLE>())
            {
                ...
                if (pname.find("NVIDIA") != string::npos)
                {
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 10", "GTX1000", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 9", "GTX900", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 8", "GTX800", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 7", "GTX700", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 6", "GTX600", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 5", "GTX500", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"GTX 4", "GTX400", archName);
                }
                else if (pname.find("AMD") != string::npos)
                {
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"gfx901",   "GCN5", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"gfx900",   "GCN5", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Ellesmere","GCN4", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Baffin",   "GCN4", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Tonga",    "GNC3", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Fiji",     "GNC3", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"hawaii",   "GNC2", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Bonaire",  "GNC2", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Oland",    "GNC1", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Cape Verde","GNC1", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Pitcairn", "GNC1", archName);
                    PARNIANMINER_MAKE_ARCH_NAME(deviceName,"Tahiti",   "GNC1", archName);
                }
...
unsigned long long parnian_xgetbv(unsigned int index)
{
...
void GetPhysicalCoreCount(U32& processorCoreCount, U32& logicalProcessorCount)
{
    ...
    PARNIANMINER_ASSERT(returnLength);
    buffer = (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION)malloc(returnLength);
    DWORD rc = GetLogicalProcessorInformation(buffer, &returnLength);
    PARNIANMINER_ASSERT(rc == 1);
    ptr = buffer;
...
#if defined(PARNIANMINER_ENABLE_SSE4) && !defined(PARNIANMINER_COND_SSE4)
    if (!CpuInfos.sse4_1Supportted)
    {
        PrintOutCritical("SSE4 is not supported by this CPU. Please use parnianminer for oldgen cpu\n");
        exit( 1);
    }
#endif
}
