...
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_BOOL(g_DisableAutoReconnect, false);
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_INT(g_maxConsecutiveSubmitErrors, 10);
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_BOOL(g_forceSequentialNonce, false)
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_BOOL(g_disableCachedNonceReuse, false)
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_STRING(g_extraPayload, "")
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_INT(g_apiPort, 0)
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_BOOL(g_apiRemote, false)
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_STRING(g_apiPW, "")
PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_INT(g_workTimeout, 60);
...
{
    Guard g(m_portMutex);
    return m_active ? m_active->port : string();
}

// Called from the miner client thread when auto port selection wants another port.
// The new port is applied by the network thread in Preconnect().
void StratumClient::RequestPortSwitch(const string& port)
{
    {
        Guard g(m_portMutex);
        m_pendingPort = port;
    ...
    // Break the blocking read; the network thread reconnects on the new port
...
void StratumClient::Connect()
{
    ...
    if (ec)
    {
        PARNIANMINER_PRINT_EXCEPTION_EX(FormatString("Error connecting to %s", m_lastActiveHost.c_str()), "");
        m_socket.close();
        m_connected = false;
    }
...
void DevReminder()
{
    // parnianminer: donation reminder removed
}
...
void StratumClient::Reconnect(U32 preSleepTimeMS)
{
    ...
    // parnianminer: dev fee removed - no donation reminder, no dev-mode reconnect
...
void StratumClient::WorkLoop()
{
    ...
    while (m_running)
    {
        while(!m_connected)
        {
            ...
            {
                PARNIANMINER_PRINT_EXCEPTION_EX("Connection error",  _e.what());
                Reconnect(3000);
            }
            catch (...) 
            {
...
void StratumClient::OnPostConnect()
{
    ...
    PARNIANMINER_ONLY_ONCE_CODE_BEGIN()
    if (g_apiPort)
        StartMiniWeb();
    PARNIANMINER_ONLY_ONCE_CODE_END()
}
...
void StratumClient::Preconnect()
{
    ...
    if (!GlobalMiningPreset::I().IsInDevFeeMode())
    {
        // never write the password in clear in the console/log
        if (!m_soloMining)
            PrintOut("User: '%s' PW: '%s'\n", m_active->user.c_str(), m_active->pass.length() ? "****" : "");
        else
            PrintDonationMsg();
    }

    if (IsSoloMining())
        PrintOut("Solo mining on node %s\n", m_active->HostDescr());
    else
    {
        PrintOut("Connecting to pool %s\n", m_active->HostDescr());
        if (m_active->autoPort)
        {
            int t = FindPoolPortTier(m_active->port);
            PrintOut("Auto port: active port is %s%s%s\n", m_active->port.c_str(),
                     t >= 0 ? " - " : "", t >= 0 ? g_poolPortTiers[t].descr : "");
        }
    }
    ...
    if (error)
    {
        PARNIANMINER_PRINT_EXCEPTION_EX(FormatString("Error. Could not connect to server %s", m_active->HostDescr()), "Retrying...");
        Reconnect(5 * 1000);
    }
    else
    {
...
void StratumClient::ProcessExtranonce(Json::Value& responseObject)
{
    ...
    PARNIANMINER_ASSERT(params.isArray());
...
bool StratumClient::ValidateNewWork(ParnianWorkSptr& work)
{
...
void StratumClient::SendWorkToMiners(ParnianWorkSptr wp)
{    
...
bool StratumClient::ProcessMiningNotify(Json::Value& params)
{
...

    {
        ...
        {
            ParnianWorkSptr newWork = InstanciateWorkPackage();
...
void StratumClient::PrepareWorkInternal(ParnianWorkSptr wp)
{
    wp->UpdateHeader();
...
void StratumClient::InitializeWP(ParnianWorkSptr wp)
{
    //because parnian have no prev_hash, we put something unique in there
...
{
    // parnianminer: no developer fee - 100% of the hashrate mines for the user
}
...
void StratumClient::RespondAuthorize(Json::Value& responseObject, U64 gpuIndex)
{
    ...
    if (!m_authorized)
    {
        ...
        if (m_active != &m_devFee)
        {
            PARNIANMINER_EXIT_APP("Connection refused");
        }
        else
        {
...
void StratumClient::ProcessMiningNotifySolo(Json::Value& jsondata)
{
    ...
    if (block != m_lastBlock || timestamp != m_lastTimestamp)
    {
        ...
        strncpy(LocalPayloadData, "parnianminer.parnianminer.parnianminer.parnianminer.parnianminer.parnianminer.parnianminer.parnianminer", sizeof(LocalPayloadData)-1);
        ...
        if (payload.length() > 52)
        {
            ...
            PARNIANMINER_EXIT_APP("Error. Deamon/Wallet miner name is too long. Set a name under 26 caracters.\nNOTE, if this error persist, just restart the demaon/wallet.");
        }
        else
        {
...
void StratumClient::CallSubmit(SolutionSptr solution)
{
    ...
    string params;
    ParnianWorkPackage* cbwp = solution->m_work.get();
    PARNIANMINER_ASSERT(cbwp);
    ...
    U64 currentNonce = solution->GetCurrentEvaluatingNonce();
    PARNIANMINER_ASSERT(currentNonce <= U32_Max);
    ...
    else
    {
        ...
        PARNIANMINER_ASSERT(cbwp->m_nonce2 != U32_Max);
...
ParnianWorkSptr StratumClient::InstanciateWorkPackage(ParnianWorkSptr* cloneFrom)
{
    if (cloneFrom)
    {
        ParnianWorkPackage* cloned = (*cloneFrom).get();
        PARNIANMINER_ASSERT(cloned);
        return ParnianWorkSptr(new ParnianWorkPackage(*cloned));
    }
    else
    {
        return ParnianWorkSptr(new ParnianWorkPackage(IsSoloMining()));
    }    
}
