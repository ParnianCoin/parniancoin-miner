...
public:
    ...
    { 
        PARNIANMINER_ASSERT(gpuAbsIndex < (int)PARNIANMINER_ARRAY_COUNT(accepts));
        accepts[gpuAbsIndex]++;
    }
...
public:
    ...
    virtual void RequestNewWork(ParnianWorkSptr wp, GenericCLMiner* miner) = 0;
...
protected:
    virtual bool    init(const ParnianWorkSptr& work) = 0;
    virtual bool    ShouldInitialize(const ParnianWorkSptr& work) = 0;

    ParnianWorkSptr  GetWork() const { Guard l(m_workMutex); return m_workTemplate; }
    ...
    ParnianWorkSptr  m_workTemplate;
