...
void Farm::SetWork(ParnianWorkSptr _wp)
{
...
bool Farm::start()
{
    ...
    {
        PARNIANMINER_RETURN_ON_EXIT_FLAG_EX(false);
        if (gpu.enabled)
        {
            PARNIANMINER_ASSERT(gpu.initialized);

            GlobalMiningPreset::CreatorClasType createType = GlobalMiningPreset::ClassOpenCL;
            gpu.gpuName = FormatString("GPU%d", globalIndex);
            if (PARNIANMINER_TEST_BIT(gpu.gpuType, GpuType_NVIDIA))
                createType = GlobalMiningPreset::ClassNvidia;
            else if (PARNIANMINER_TEST_BIT(gpu.gpuType, GpuType_AMD))
                createType = GlobalMiningPreset::ClassOpenCL;
            else if (PARNIANMINER_TEST_BIT(gpu.gpuType, GpuType_CPU))
            {
                createType = GlobalMiningPreset::ClassCPU;
                gpu.gpuName = "CPU";
            }

            if (PARNIANMINER_TEST_BIT(gpu.gpuType, GpuType_AMD))
            {
...
{
    PARNIANMINER_ASSERT(m_reconnect);
    m_reconnect(gpuAbsIndex);
}


void Farm::RequestNewWork(ParnianWorkSptr wp, GenericCLMiner* miner)
{
    PARNIANMINER_ASSERT(m_requestNeWork);
...
{
   PARNIANMINER_ASSERT(m_onSolutionFound);
...
{ 
    ...
    else
    {
        ...
        {
            PARNIANMINER_EXIT_APP("To many consecutive submit errors.");
        }
    }
