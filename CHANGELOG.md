...
*New: automatic pool port selection. Use "auto" instead of the port: -s stratum.parniancoin.com:auto
 ...
 A numeric port keeps the old behaviour (the miner stays on that port). Solo mining is unchanged.
*New: welcome message from the ParnianCoin team at startup
*Help (-help), config.txt, scripts and README rewritten for ParnianCoin (pool ports, auto mode, solo node)
*Security: fixed a one-byte stack overflow in the remote API request reader
*Security: remote API is disabled by default and listens on 127.0.0.1 only (new option -apiremote)
*Security: constant-time API password check, API requests and pool password no longer written to the log
*Security: fixed crash on empty miner_file request
*Antivirus: the console window is never hidden anymore (processpriority 0)
*Antivirus: Windows executable now carries version information and icon
*Packaging: no dependency to install anymore. jsoncpp (bundled copy), boost and the C++ runtime are linked statically.
 Windows builds only use system DLLs. The portable Linux build runs on glibc 2.17+ (Ubuntu 16.04+, CentOS 7+).
*Build: cross-compile toolchain files for Windows 64/32 bit (mingw-w64) and portable Linux (zig) in CMake/
...
Version 2.2b - Removed parnianpool.org from DevPool list
...
Version 2.1
* Optimized RandomHash2 with up to 80% more speed. (On some system parnianminer 2.1 is up to 250% faster than official miner)
* Fixed linux binary where main thread was exiting after miner startup.
* Linux zip files now contain parnianminer as an executable and not a simple file.
...
* Added ETHman and AwesomeMiner support (miner_getstat1, miner_restart, miner_reboot, miner_file, control_gpu)
  It is strongly recommented to NOT put parnianminer in a loop in a script if you intend to use EthMan or AwseomeMiner.
  The restart option will close the instance and restart a new one. So if parnianminer is in a loop, it will start another new instance everytime.
* Added Parnian Coin ScreenSaver V1.0 - Windows
* Now starting parnianminer without parametres will automatically use config.txt.
* Added support Maxwell architecture 5.3 (Tegra Tegra (Jetson) TX1 / Tegra X1) on windows
* Re-added Parnian architecture 6.0 (Quadro GP100, Tesla P100, DGX-1) that was mistakenly removed in 1.4
* Re-added Parnian architecture 6.2 ( Integrated GPU on the NVIDIA Drive PX2, Tegra (Jetson) TX2) that was mistakenly removed in 1.4
...
* Fix : On linux, parnianminer stall for ever when network is lost (cable unplug or switch/router crashes). 
* Fix : On very rare occasions, parnianminer would stall at startup.
...
* Graceful handling of recurrent assert (m_fullHeader.size() == ParnianHeaderSize)
