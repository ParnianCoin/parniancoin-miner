    ...
    else if (esmode == eShutdownFull)
    {
        ...
        auto t = new std::thread([&]()
        {
            ...
            if (AtomicGet(g_shutdownDone) == 0)
                PARNIANMINER_EXIT_APP("Shutdown takes to long, exiting miner now.\n");
...
void ClientManager::Initialize()
{
    ...
    if (!gpuSwitchPresent)
    {
        for(U32 i = 0; i < GpuManager::Gpus.size(); ++i)
        {
            ...
            if (PARNIANMINER_TEST_BIT(g.gpuType, GpuType_CPU))
            {
                g_useCPU = true;
...
#endif
        PARNIANMINER_ASSERT(GpuManager::CpuInfos.numberOfProcessors != 0);
