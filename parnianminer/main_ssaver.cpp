...
{
    HKEY hKey;
    string regPath = "Software\\ParnianCoin\\ScreenSaver";
...
string ReadRegistryOptions(string& serverPort, string& user, string& cpuCount, string& extra, U32& nonceGraphic)
{
    nonceGraphic = 0;
    HKEY hKey;
    string defOpt = "-apiport 0 -v 2 -r 100 -s YOUR_POOL_HOST:PORT -su YOUR_PARNIAN_ACCOUNT -cpu -cputhreads 2 -processpriority 1";
    string baseOpt = "-apiport 0 -processpriority 1 -cpu ";
    string defExtra = "-v 2 -r 100 ";
    string defServerPort = "mine.pool.parnianpool.org:3333";
    string defUSer = "YOUR_PARNIAN_ACCOUNT";
    string defCpus;
    ...
    for (int i = 0; i < PARNIANMINER_ARRAY_COUNT(NonceGraphicNames); i++)
    {
...
void Initialize()
{
    if (!inited)
    {
        ...
        *errorInfo = 0;
        ...
        if (!fChildPreview)
        {
            //Detect parnianminer already running !!!
            if (!FindProcess("parnianminer.exe"))
            {
...
LRESULT WINAPI ScreenSaverProc(HWND hWnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_CREATE:
    {
        ...
        if (fChildPreview)
        {
            ...
            LPCTSTR lpszClassName = TEXT("ParnianCoinScreenSaverC");
            WNDCLASSEX wc;
            ...
            if (minerIsRunning)
            {
                ...
                if (TimeGetMilliSec() > nextClear)
                {
                    ...
                    if (nextClear == 0)
                    {
                        if (currentNonceGraphic != C_NonceGraphic_FullBlack)
                        {
                            const char* str = FormatString("PARNIANminer is allready running...\n");
                            TextOutA(backDC, 20, 20, str, (int)strlen(str));
                        }
                    }
