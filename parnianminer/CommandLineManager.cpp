...
S32 PARNIANMINER_ValidateGlobalVarRange(const char* varName, const string& valStr, S32 _min, S32 _max)
{
    S32 val = 0;
...
void CmdLineManager::RegisterValue(const string& _symbol, const string& cathegory,  const string& descriptor, ValFunc f)
{
    ...
    PARNIANMINER_ASSERT(Find(symbol) == NULL);
...
{
    PARNIANMINER_ASSERT(switchName[0] == '-');
...
{
    printf("\nparnianminer will automatically load config.txt if no commandline are present\n\n");
...
bool CmdLineManager::Parse(const strings& strList, bool exitOnError)
{
    ...
    auto AddArgs = [&] (const char* str)
    {
        ...
        PARNIANMINER_ASSERT(maxLen < 511);
