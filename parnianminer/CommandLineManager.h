...
#define PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_STRING(OPTION_NAME, VAR_NAME, VAR_CATEGORY, VAR_DESCR) \
...
#define PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_STRING(VAR_NAME, INIT_VAL)  \
    string VAR_NAME = INIT_VAL; _CmdLineManagerTrigger##VAR_NAME<string> _trigger##VAR_NAME(&VAR_NAME); 


//define a global var as {VAR_NAME} that is registered in the command line manager from pre-main scope
#define PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_INT(OPTION_NAME, VAR_NAME, VAR_CATEGORY, VAR_DESCR, VAL_MIN, VAL_MAX) \
    extern int VAR_NAME; \
    template <typename T> struct _CmdLineManagerTrigger##VAR_NAME \
    {  \
        T* _var; \
        ...
        {  \
            _var = v; \
            CmdLineManager::GlobalOptions().RegisterValue(OPTION_NAME, VAR_CATEGORY, VAR_DESCR, [&](const string& val){ *_var = PARNIANMINER_ValidateGlobalVarRange(OPTION_NAME, val, VAL_MIN, VAL_MAX);}); \
        } \
    }; extern _CmdLineManagerTrigger##VAR_NAME<int> _trigger##VAR_NAME; \


#define PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_INT(VAR_NAME, INIT_VAL)  \
    int VAR_NAME = INIT_VAL; _CmdLineManagerTrigger##VAR_NAME<int> _trigger##VAR_NAME(&VAR_NAME); 



//define a global var as {VAR_NAME} that is registered in the command line manager from pre-main scope
#define PARNIANMINER_COMMAND_LINE_DECLARE_GLOBAL_BOOL(OPTION_NAME, VAR_NAME, VAR_CATEGORY, VAR_DESCR) \
...
#define PARNIANMINER_COMMAND_LINE_DEFINE_GLOBAL_BOOL(VAR_NAME, INIT_VAL) \
...
extern S32 PARNIANMINER_ValidateGlobalVarRange(const char* varName, const string& valStr, S32 _min, S32 _max);
